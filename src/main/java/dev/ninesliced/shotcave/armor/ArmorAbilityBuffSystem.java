...
public final class ArmorAbilityBuffSystem {
    ...
    public static void activateAbility(@Nonnull Ref<EntityStore> ref,
        ...
        LOGGER.info("[Shotcave] Armor ability activated: " + ability.getDisplayName()
        ...
        // Apply visual effect for the buff duration
        applyBuffEffect(ref, accessor, ability.getDurationSeconds());

        switch (ability) {
            case BERSERKER -> {} // +40% damage read by ShotcaveDamageInteraction/MeleeDamageEffectSystem
            case REGENERATION -> {} // HP regen ticked in ArmorChargeSystem
            case GUARDIAN -> {} // 50% damage reduction read by DungeonLethalDamageSystem
            case PURIFICATION -> {
                // Cleanse active DoT effects
                accessor.tryRemoveComponent(ref,
                        dev.ninesliced.shotcave.systems.DamageEffectComponent.getComponentType());
            }
            case SWIFTNESS -> applySpeedBuff(ref, accessor, 1.5f);
            case WARDEN -> {} // +25% spike damage read by DungeonLethalDamageSystem
            default -> {}
        }
    }
    ...
    public static void expireBuff(@Nonnull Ref<EntityStore> ref,
                                  @Nonnull ComponentAccessor<EntityStore> accessor,
                                  @Nonnull ArmorSetAbility ability) {
        LOGGER.info("[Shotcave] Armor ability expired: " + ability.getDisplayName());
